#!/usr/bin/env python3
"""
Full script for text classification (Sensitive vs Insensitive)
Follows the NLP tutorial: TF-IDF + Logistic Regression / Multinomial NB
With text preprocessing: lowercase, punctuation removal, stopword removal, and lemmatization.
"""
...
for name, model in models.items():
    if hasattr(model, "predict_proba"):
        ...
        fpr, tpr, _ = roc_curve(
            y_test.map(lambda x: 1 if x == "SENSITIVE" else 0),
            probs
        )
        roc_auc = auc(fpr, tpr)
