...
class PresidioClassifier(ClassifierInterface):
    def __init__(self,
        ...
        # 4) build Presidio Pattern objects from your content‐classifier style regexes
        kw_patterns = []
        for kw in keywords:
            esc = re.escape(kw)
        ...
        class DiseaseSynonymRecognizer(PatternRecognizer):
            def __init__(self, min_score=0.2):
                ...
                # Remove "disease" itself to avoid duplication if desired
                # disease_synonyms.discard("disease")
                patterns = [
                    Pattern(
                        name=syn,
