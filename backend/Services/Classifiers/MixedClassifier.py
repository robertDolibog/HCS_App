...
class MixedClassifier(ClassifierInterface):
    def __init__(self,
        ...
        #  1) wrap each keyword in custom boundaries so "SSN" doesn't match "passportssn"
        #  2) include any user‐provided patterns verbatim
        patterns = []
        ...
        for pat in self.config.get("sensitive_patterns", []):
            patterns.append(pat)
